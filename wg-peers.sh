#!/usr/bin/env bash
# wg-peers.sh — Handy WireGuard peer manager.
#
...
#
# Usage:
#   wg-peers.sh <command> [arguments] [options]
# 
# See below or run with -h/--help for details.
#
# Notes:
...
usage() {
  cat <<'EOF'
Usage: sudo bash ./wg-peers.sh <command> [args]
...
Commands:
  add <name>
      ...
      [--print-psk]                    (print the PSK to stdout after creating the peer)

#   add <name> [options]           - add a peer (keys, server config, client files)
#   list [--iface] [--psk ...]     - list peers and status (optional PSK display)
#   show <name>                    - show client config/template/peer-info
#   qr <name>                      - display QR for full client config (needs PrivateKey)
#   disable <name>                 - disable peer in server config (retain artifacts)
#   enable <name>                  - enable a previously disabled peer
#   revoke <name> [--purge]        - remove peer from server; --purge deletes client files
...
lock_or_die() {
  ...
  if have_cmd flock; then
    exec 200>"$lockfile"
    flock -n 200 || die "Another wg-peers.sh is running for iface ${IFACE}."
  else
...
peer_psk_from_block() {
  ...
  awk -v n="$name" '
    $0 ~ "^# BEGIN_PEER[[:space:]]+" n "$" {inblk=1}
    inblk && $1=="PresharedKey" && $2=="=" {print $3; exit}
    inblk && $0 ~ "^#PresharedKey[[:space:]]*=" {
      line=$0; sub("^#","",line); gsub(/[[:space:]]+/," ",line); split(line,a," "); print a[3]; exit
    }
    $0 ~ "^# END_PEER[[:space:]]+" n "$" {inblk=0}
  ' "$(conf_path)"
...
# ${name} — generated by wg-peers.sh on $(date -Is)
[Interface]
PrivateKey = ${priv}
Address = ${ip4}/32
DNS = ${dns}
...
cmd_add() {
  ...
  if [[ $print_psk -eq 1 ]]; then
    if [[ -f "${d}/psk" ]]; then
      echo "PSK ${name}: $(cat "${d}/psk")"
    else
      echo "PSK ${name}: <none>"
    fi
  fi
...
cmd_list() {
  ...
  if [[ -n "${psk_names}" ]]; then
    ...
    while IFS= read -r n; do
      [[ -z "$n" ]] && continue
      sanitize_name "$n"
      psk_file="$(client_dir "$n")/psk"
      psk_val=""
      if [[ -f "$psk_file" ]]; then
        psk_val="$(cat "$psk_file")"
      else
        psk_val="$(peer_psk_from_block "$n" || true)"
      fi
      [[ -n "$psk_val" ]] || psk_val="<none>"
      printf "%-20s %s\n" "$n" "$psk_val"
    done < <(echo "${psk_names}" | tr ',' '\n' | sed -E 's/^[[:space:]]+|[[:space:]]+$//g' | grep -v '^$')
  fi
}
