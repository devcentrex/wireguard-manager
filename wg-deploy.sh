#!/usr/bin/env bash
# wg-server-deploy.sh — WireGuard server deployment for Ubuntu 22.04 (NAT internet breakout).
#
# What it does:
...
#     --iface wg0 --subnet 10.1.1.0/24 --server-ip 10.1.1.1 \
#     --port 51820 --endpoint <host name or IP> \
#     --dns 1.1.1.1
#
# Notes:
...
Deploy options:
  ...
  --subnet <CIDR>         VPN subnet for clients (default: 10.1.1.0/24)
  --server-ip <IP>        Server VPN IP inside subnet (default: 10.1.1.1)
...
Example (your scenario):
  sudo bash ./wg-server-deploy.sh deploy \
    --iface wg0 --subnet 10.1.1.0/24 --server-ip 10.1.1.1 \
    --port 51820 --endpoint <host name or IP> --dns 1.1.1.1
EOF
}
...
main() {
  ...
  case "$cmd" in
    deploy)
      ...
      echo "  sudo bash ./wg-deploy2.sh add alice --iface ${IFACE} --endpoint ${ENDPOINT_HOST}:${PORT} --ip4 10.1.1.2 --client-allowed \"0.0.0.0/0,::/0\" --dns \"${DNS}\""
      ;;
