#!/usr/bin/env bash
# wg-deploy.sh — WireGuard server deployment for Ubuntu 20+ (NAT internet breakout).
#
...
write_server_conf() {
  ...
  cat >"$conf" <<EOF
# Generated by wg-deploy.sh on $(date -Is)
...
usage() {
  cat <<'EOF'
Usage:
  sudo bash ./wg-deploy.sh deploy [options]
...
main() {
  ...
  case "$cmd" in
    deploy)
      ...
      echo
      echo "Next: add peers using your management script wg-peers.sh"
      ;;
